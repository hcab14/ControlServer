...
public:
    ...
    quint32 readRegister(quint32 address) {
      ...
      IPBusPacket p;
      p.addTransaction(this, read, address, nullptr, 1);
...
protected:
  ...
  bool transceive(IPBusPacket &p, bool shouldResponseBeProcessed = true) { //send request, wait for response, receive it and check correctness
    if (p.requestSize() <= 1) {
    ...
    if (n == 0) {
      ...
      bool result = shouldResponseBeProcessed ? p.processResponse(this) : true;
      return result;
    }
  }
...
public slots:
    ...
    void checkStatus() {
      ...
      if (!qsocket->waitForReadyRead(100) && !qsocket->hasPendingDatagrams()) {
        ...
        qint32 n = qsocket->read((char *)&p.statusResponse(), qsocket->pendingDatagramSize());
        if (n != sizeof (p.statusResponse()) || p.statusResponse().header != p.statusRequest().header) {
          isOnline = false;
  ...
  void writeRegister(quint32 address, quint32 data, bool syncOnSuccess = true) {
    IPBusPacket p;
    writeRegister(p, address, data, syncOnSuccess);
  }
  void writeRegister(IPBusPacket &p, quint32 address, quint32 data, bool syncOnSuccess = true) {
    p.addTransaction(this, write, address, &data, 1);
  ...
  void clearBit(quint8 n, quint32 address, bool syncOnSuccess = true) {
    IPBusPacket p;
    p.addTransaction(this, RMWbits, address, masks(~(1 << n), 0x00000000));
    if (transceive(p) && syncOnSuccess) sync();
  }
  ...
  void writeNbits(quint32 address, quint32 data, quint8 nbits = 16, quint8 shift = 0, bool syncOnSuccess = true) {
    ...
    IPBusPacket p;
    p.addTransaction(this, RMWbits, address, masks( ~quint32(mask << shift), quint32((data & mask) << shift) ));
    if (transceive(p) && syncOnSuccess) sync();
  }
};
